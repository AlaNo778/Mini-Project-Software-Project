...
$query = "
    SELECT p.pf_fname, p.pf_lname, COUNT(s.std_id) AS student_count 
    FROM professor p
    LEFT JOIN student s ON p.pf_id = s.pf_id
    GROUP BY p.pf_id, p.pf_fname, p.pf_lname
    ORDER BY p.pf_lname ASC;
";
...
while ($row = mysqli_fetch_assoc($result)) {
    $pf_id = isset($row['pf_id']) ? $row['pf_id'] : 0; // ป้องกันค่า NULL
    $advisorData .= '<tr>
        <td>' . htmlspecialchars($row['pf_fname'] . ' ' . $row['pf_lname']) . '</td>
        <td>' . htmlspecialchars($row['student_count']) . '</td>
        <td>
            <a href="coordinator_assign_advisorS.php?id=' . $pf_id . '">
...
<h2 class = "as">กำหนดอาจารย์ที่ปรึกษา</h2>
<nav class="breadcrumb">
    <a href="coordinator_dashboard.php" class="breadcrumb-item">Home</a>
    <span class="breadcrumb-item active">กำหนดอาจารย์ที่ปรึกษา</span>
</nav>
